...
function emptyOrder(existingOrders = []) {
  return {
    id: genId(),
    order_number: genOrderNumber(existingOrders),
    name: '',
    status: 'draft',
    customer_name: '',
    contact_name: '',
...
export default function OrderModal({ order, existingOrders, pricebooks, onSave, onClose }) {
  const activePricebooks = (pricebooks || []).filter((pb) => pb.active);
  const defaultPb = activePricebooks.find((pb) => pb.is_default);
  const initialOrder = order || emptyOrder(existingOrders);
  if (!order && !initialOrder.pricebook_id && defaultPb) {
    initialOrder.pricebook_id = defaultPb.id;
  }
  ...
  const handleTermChange = (term) => {
    ...
    setF((p) => ({ ...p, term_months: t, end_date: calcEndDate(p.start_date, t) }));
  };

  const handleStartChange = (date) => {
    setF((p) => ({ ...p, start_date: date, end_date: calcEndDate(date, p.term_months) }));
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal modal-theme-orders" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="field">
          ...
          <input className="field-input" value={f.name} onChange={(e) => s('name', e.target.value)} placeholder="e.g. Riot Games — Enterprise Renewal" autoFocus />
        </div>
        ...
        <div className="modal-section">
          ...
          <div className={`modal-section-content ${openSections[SECTIONS.TERM] ? 'is-open' : ''}`}>
            <div className="grid-2">
              <div className="field">
                <label className="field-label">Order Expiration Date</label>
          ...
          <div className={`modal-section-content ${openSections[SECTIONS.INTERNAL] ? 'is-open' : ''}`}>
            <div className="grid-2">
              <div className="field">
                ...
                <input className="field-input" value={f.partner_name || ''} onChange={(e) => s('partner_name', e.target.value)} placeholder="Leave blank if direct" />
              </div>
              <div className="field">
...
export { emptyOrder, genOrderNumber };
