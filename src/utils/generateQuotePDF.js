...
// TODO: Embed Poppins Bold as base64 font — using Helvetica Bold as fallback
// TODO: Embed Mulish as base64 font — using Helvetica as fallback
// TODO: Embed Roboto Mono as base64 font — using Courier as fallback
...
export function generateQuotePDF(quote, products, settings, { preview = false } = {}) {
  ...
  // ── PAGE 1: HEADER ──
  // Top left: Netlify logo text (fallback since we can't embed SVG easily)
  doc.setFont(FONT_HEADING, 'bold');
  doc.setFontSize(18);
  doc.setTextColor(...COLOR_TEAL);
  doc.text('netlify', MARGIN, y + 2);

  // Top right: Quote info
  doc.setFont(FONT_HEADING, 'bold');
  doc.setFontSize(14);
  ...
  if (packageLines.length > 0) {
    ...
    packageLines.forEach((pkg) => {
      ...
      if (platformSubs.length > 0) {
        ...
        y = drawSubEyebrow(doc, 'Platform', MARGIN + 8, y);
        platformSubs.forEach((s) => {
          y = checkPage(doc, y, 8);
          doc.setFont(FONT_BODY, 'bold');
          doc.setFontSize(10);
          doc.setTextColor(...COLOR_TEXT);
          doc.text(s.product_name, MARGIN + 12, y);
          y += 5;
        });
        y += 3;
      }
      ...
      if (entitlementSubs.length > 0) {
        y = checkPage(doc, y, 15);
        y = drawSubEyebrow(doc, 'Entitlements', MARGIN + 8, y);
        entitlementSubs.forEach((s) => {
          y = checkPage(doc, y, 8);
          doc.setFont(FONT_BODY, 'bold');
          doc.setFontSize(10);
          doc.setTextColor(...COLOR_TEXT);
          const qtyStr = s.quantity > 1 ? `  (${fmtQty(s.quantity)})` : '';
          doc.text(`${s.product_name}${qtyStr}`, MARGIN + 12, y);
          y += 5;
        });
        y += 3;
      }
      ...
      if (supportSubs.length > 0) {
        y = checkPage(doc, y, 15);
        y = drawSubEyebrow(doc, 'Support', MARGIN + 8, y);
        supportSubs.forEach((s) => {
          y = checkPage(doc, y, 8);
          doc.setFont(FONT_BODY, 'bold');
          doc.setFontSize(10);
          doc.setTextColor(...COLOR_TEXT);
  ...
  if (standaloneEntitlements.length > 0) {
    y = checkPage(doc, y, 40);
    y = drawEyebrow(doc, 'Additional Entitlements', MARGIN, y);

    const entHead = [['', 'QTY', 'MONTHLY LIST PRICE', 'MONTHLY DISCOUNTED PRICE', 'MONTHLY AMOUNT']];
    const entBody = standaloneEntitlements.map((l) => [
      l.product_name,
      fmtQty(l.quantity),
      fmtCurrency(l.list_price || 0),
      fmtCurrency(l.net_price || l.list_price || 0),
      fmtCurrency(calcLineExtended(l)),
    ]);
    const entSubtotal = standaloneEntitlements.reduce((s, l) => s + calcLineExtended(l), 0);
    ...
    autoTable(doc, {
      ...
      theme: 'plain',
      styles: { fontSize: 9, cellPadding: 4, textColor: COLOR_TEXT, lineColor: COLOR_DIVIDER, lineWidth: 0.3 },
      headStyles: { fillColor: [245, 245, 245], textColor: COLOR_MUTED, fontStyle: 'bold', fontSize: 8, font: FONT_MONO },
      columnStyles: { 0: { cellWidth: 'auto' }, 1: { halign: 'center', cellWidth: 22 }, 2: { halign: 'right', cellWidth: 38 }, 3: { halign: 'right', cellWidth: 42 }, 4: { halign: 'right', cellWidth: 34 } },
    });
    y = doc.lastAutoTable.finalY + 4;
    doc.setFont(FONT_BODY, 'bold');
    doc.setFontSize(10);
    doc.setTextColor(...COLOR_TEXT);
    doc.text(`${fmtCurrency(entSubtotal)} / month`, pageWidth - MARGIN, y, { align: 'right' });
    y += 10;
  }
  ...
  if (standaloneAddons.length > 0) {
    ...
    const addonHead = [['', 'MONTHLY LIST PRICE']];
    const addonBody = standaloneAddons.map((l) => [l.product_name, fmtCurrency(l.net_price || l.list_price || 0)]);
    const addonSubtotal = standaloneAddons.reduce((s, l) => s + calcLineExtended(l), 0);
    ...
    doc.setFontSize(10);
    doc.setTextColor(...COLOR_TEXT);
    doc.text(`${fmtCurrency(addonSubtotal)} / month`, pageWidth - MARGIN, y, { align: 'right' });
    y += 10;
  }

  // ── CONSUMPTION LIMITS & OVERAGE RATES ──
  const overageRows = [];
  // Gather entitlement/seat/credit lines (from packages and standalone)
  ...
  allEntLines.forEach((l) => {
    ...
    seen.add(key);
    const included = fmtQty(l.quantity);
    let overage = '';
    ...
    overageRows.push([key, included, overage || '\u2014']);
  });
  ...
  if (overageRows.length > 0) {
    ...
    autoTable(doc, {
      startY: y,
      head: [['', 'INCLUDED (MONTHLY)', 'OVERAGE RATES (MONTHLY)']],
      body: overageRows,
      margin: { left: MARGIN, right: MARGIN },
      theme: 'plain',
      styles: { fontSize: 9, cellPadding: 4, textColor: COLOR_TEXT, lineColor: COLOR_DIVIDER, lineWidth: 0.3 },
      headStyles: { fillColor: [245, 245, 245], textColor: COLOR_MUTED, fontStyle: 'bold', fontSize: 8, font: FONT_MONO },
      columnStyles: { 0: { cellWidth: 'auto' }, 1: { halign: 'center', cellWidth: 40 }, 2: { halign: 'center', cellWidth: 45 } },
    });
  ...
  // Calculate pre-discount annual
  const priceableLines = allLines.filter((l) => {
    if (l.parent_line_id) return l.price_behavior === 'related';
    return true;
  });
  const totalListAnnual = priceableLines.reduce((s, l) => s + (l.list_price || 0) * (l.quantity || 1) * 12, 0);
  const discountAmount = totalListAnnual - totals.annual;

  const totalsData = [];
  totalsData.push(['Total Annual List Price', fmtCurrency(totalListAnnual)]);
  if (discountAmount > 0) totalsData.push(['Discount', `-${fmtCurrency(discountAmount)}`]);
  totalsData.push(['Net Annual Fees', fmtCurrency(totals.annual)]);
  ...
  autoTable(doc, {
    ...
    theme: 'plain',
    styles: { fontSize: 10, cellPadding: 3, textColor: COLOR_TEXT },
    columnStyles: {
      0: { fontStyle: 'normal', textColor: COLOR_MUTED, cellWidth: 50 },
  ...
  // Gather product-level terms from line items
  const seenProducts = new Set();
  allLines.forEach((l) => {
    if (seenProducts.has(l.product_id)) return;
    seenProducts.add(l.product_id);
    const prod = productMap.get(l.product_id);
    if (prod?.terms && prod.terms.trim()) {
      productTerms.push({ name: prod.name, terms: prod.terms.trim() });
    }
  });
  ...
  if (hasTerms) {
    ...
    // 2. Product-specific exhibits
    const exhibitLetters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    productTerms.forEach((pt, i) => {
      y = checkPage(doc, y, 20);
      const letter = exhibitLetters[i] || String(i + 1);
      doc.setFont(FONT_HEADING, 'bold');
      doc.setFontSize(11);
      doc.setTextColor(...COLOR_TEXT);
      doc.text(`Exhibit ${letter} \u2014 ${pt.name}`, MARGIN, y);
      y += 6;
      doc.setFont(FONT_BODY, 'normal');
      doc.setFontSize(10);
      doc.setTextColor(...COLOR_TEXT);
      const termLines = doc.splitTextToSize(pt.terms, contentWidth);
    ...
    // 3. Quote-level additional terms
    if (quote.terms_conditions && quote.terms_conditions.trim()) {
      ...
      doc.setFontSize(10);
      doc.setTextColor(...COLOR_TEXT);
      const atLines = doc.splitTextToSize(quote.terms_conditions.trim(), contentWidth);
      atLines.forEach((line) => {
        y = checkPage(doc, y, 6);
        doc.text(line, MARGIN, y);
        y += 4.5;
      });
